...
public typealias MenuDataType = (text: String, icon: UIImage?, isSticky: Bool)
public typealias SelectionClosure = (Index, MenuDataType) -> Void
public typealias MultiSelectionClosure = ([Index], [MenuDataType]) -> Void
public typealias ConfigurationClosure = (Index, MenuDataType) -> MenuDataType
public typealias CellConfigurationClosure = (Index, MenuDataType, DropDownCell) -> Void
...
public final class DropDown: UIView {
  ...
  /// The value that indicating which side the menu should be aligned with. The default value is left.
  ...
  */
  ...
  public var sideAlignment = SideAlignment.left
  ...
  */
  public var localizationKeysDataSource = [MenuDataType]() {
    didSet {
      dataSource = localizationKeysDataSource.map { (NSLocalizedString($0.text, comment: ""), $0.icon, $0.isSticky)}
    }
  }
  ...
  */
  public convenience init(anchorView: AnchorView, selectionAction: SelectionClosure? = nil, dataSource: [MenuDataType] = [], topOffset: CGPoint? = nil, bottomOffset: CGPoint? = nil, cellConfiguration: ConfigurationClosure? = nil, cancelAction: Closure? = nil) {
    self.init(frame: .zero)
...
extension DropDown {
  ...
  fileprivate func computeLayout() -> (x: CGFloat, y: CGFloat, width: CGFloat, offscreenHeight: CGFloat, visibleHeight: CGFloat, canBeDisplayed: Bool, Direction: Direction) {
    ...
    if case .right = sideAlignment, let anchorView = anchorView {
      layout.x = layout.x - layout.width + anchorView.plainView.bounds.width
    }
  ...
  fileprivate func constraintWidthToBoundsIfNecessary(layout: inout ComputeLayoutTuple, in window: UIWindow) {
    ...
    // TODO(ENG-32805): Add the logic to handle the case if the menu is out of the left boundary.
    if maxX > windowMaxX {
...
extension DropDown {
  ...
  */
  ...
  public var selectedItem: (String, UIImage?, Bool)? {
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
  ...
  public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    // Makes the separator full width for cell in tableview.
  ...
  fileprivate func configureCell(_ cell: DropDownCell, at index: Int) {
    ...
    if let cellConfiguration = cellConfiguration {
      cell.optionLabel.text = cellConfiguration(index, dataSource[index]).text
      cell.optionIcon.image = cellConfiguration(index, dataSource[index]).icon
    } else {
